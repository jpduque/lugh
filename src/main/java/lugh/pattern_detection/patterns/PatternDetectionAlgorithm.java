package lugh.pattern_detection.patterns;

import lugh.pattern_detection.parser.ClassObject;
import lugh.pattern_detection.parser.ClassObject.Abstraction;
import lugh.pattern_detection.parser.Connection;
import lugh.pattern_detection.parser.Connection.Type;
...
/**
 * Basic algorithm class. Its function DetectPattern(Pattern p) is responsible for detecting lugh.pattern_detection.patterns over the given
...
public class PatternDetectionAlgorithm {
    ...
    /**
     * Detects PatternCandidates and returns the results in a String. Used mainly for lugh.pattern_detection.gui purposes.
     *
    ...
    public static List<String> DetectPattern_File(Pattern p, Boolean grouping) {
        DetectPatternFast(p);
        if (grouping) {
            findSuperCandidates();
            findHyperCandidates();
            return HyperCandidatesAsClass();
        } else {
            return PatternCandidatesAsClass();
        }
    }
    ...
    private static List<String> PatternCandidatesAsClass() {
        List<String> s = new ArrayList<>();
        for (PatternCandidate e : Candidates) {
            for (ClassObject member : e.getMembers())
                s.add(member.getName() + ".class");
        }
        return s;
    }
    ...
    private static List<String> HyperCandidatesAsClass() {
        List<String> s = new ArrayList<>();
        for (PatternCandidate e : TotalHyperCandidates) {
            s.add(e.candidatetoString() + ".class");
        }
        return s;
    }
